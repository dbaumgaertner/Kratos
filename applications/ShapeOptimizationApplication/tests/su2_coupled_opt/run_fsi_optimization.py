...
class CustomAnalyzer(AnalyzerBaseClass):
    ...
    def __init__(self):
        ...
        self.cfd_interface_output_parameters = km.Parameters("""
        {
            "result_file_configuration" : {
                ...
                "node_output"         : false,
                "skin_output"         : false,
                "nodal_results"       : ["POINT_LOAD", "NORMAL", "DF1DX", "ADJOINT_DISPLACEMENT", "MESH_CHANGE", "MESH_DISPLACEMENT", "CFD_GRADIENT"],
                "gauss_point_results" : []
            },
        ...
        self.stress_response_settings = km.Parameters("""
        {
                ...
                "step_size"                        : 1e-11,
                "critical_part_name"               : "stress_partition_for_optimization",
        ...
        self.RESTART_FLOW_FILENAME = "restart_flow.csv"
    ...
    def InitializeBeforeOptimizationLoop(self):
        ...
        # Initialize damping of gradient and mesh changes on CFD side & gradient comming from structure side (damping of mesh change on structure side is done by optimizer)
        damping_regions = {}
    ...
    def AnalyzeDesignAndReportToCommunicator(self, current_design, optimization_iteration, communicator):
        ...
        if communicator.isRequestingGradientOf("pressure_loss"):
            ...
            if parameters["optimization_settings"]["model_settings"]["damping"]["apply_damping"].GetBool():
                self.cfd_interface_damping_utils.DampNodalVariable(kso.DF1DX)
        ...
        if communicator.isRequestingValueOf("mean_stress") or communicator.isRequestingGradientOf("mean_stress"):
            ...
            if communicator.isRequestingGradientOf("mean_stress"):
                ...
                if parameters["optimization_settings"]["model_settings"]["damping"]["apply_damping"].GetBool():
                    self.cfd_interface_damping_utils.DampNodalVariable(kso.CFD_GRADIENT)
    ...
    def __RunCSM(self, opt_itr, new_mesh, displacements):
        ...
        # Apply damping on structure side
        if parameters["optimization_settings"]["model_settings"]["damping"]["apply_damping"].GetBool():
            csm_damping_regions = {}
            for regionNumber in range(parameters["optimization_settings"]["model_settings"]["damping"]["damping_regions"].size()):
                regionName = parameters["optimization_settings"]["model_settings"]["damping"]["damping_regions"][regionNumber]["sub_model_part_name"].GetString()
                csm_damping_regions[regionName] = csm_adjoint_part.GetSubModelPart(regionName)
            csm_damping_utils = kso.DampingUtilities(csm_adjoint_part, csm_damping_regions, parameters["optimization_settings"]["model_settings"]["damping"].Clone())
            csm_damping_utils.DampNodalVariable(km.SHAPE_SENSITIVITY)
